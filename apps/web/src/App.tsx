...
import { cancelDuelMatch, createDuelMatch, createGameSession, downloadRoundAudio, forfeitDuelMatch, getDuelMatch, heartbeatDuelMatch, joinDuelMatch, requestDuelRematch, submitRoundScore, trackEvent, updateDuelActivity, uploadRoundAudio, type DuelMatch } from './api';
...
export default function App() {
  ...
  useEffect(() => {
    if (!match || !token || match.status === 'cancelled') return;
    let stopped = false;
  ...
  function sync(next: DuelMatch, currentPlayer = player) {
    ...
    const isRemoteRematch = Boolean(match && (match.status === 'finished' || match.status.startsWith('forfeited_by_')) && next.status === 'round_1' && next.activity_status === 'rematch_started');
    if (isRemoteRematch) {
      localFlowLocked.current = false; loadingChallenge.current = false; loadedChallengeRound.current = null;
      remoteAudio.current = null; reversed.current = null; originals.current = [null, null]; attempts.current = [null, null];
      void deleteLocalOriginal(next.id, 1).catch(() => undefined); void deleteLocalOriginal(next.id, 2).catch(() => undefined);
      setForfeitedBy(null); setPlaybackError('');
    }
  ...
  const rematchLabel = mode !== 'remote' ? 'Реванш' : match?.rematch_requested_by === player ? 'Ждём ответа соперника…' : match?.rematch_requested_by ? '🔥 Играть ещё' : 'Предложить реванш';
